...
public class TrialBalanceDetailDAO extends BasicDAO implements ITrialBalanceDetailDAO {
	...
	public List<TrialBalanceReportDto> findTrialBalanceDetailList(TrialBalanceCriteriaDto dto) {
		...
		try {
			...
			String thisMonth = BusinessUtil.getBudgetInfo(reportDate, 3);
			String previousBudgetMonth = BusinessUtil.getBudgetInfo(previousDate, 3);
...
//			}
			...
			if(previousMonthvalue< 1 ) {
				...
				queryString.append(" ) THEN ABS(CC." + reportBudgetMonth + ") ELSE 0 END AS mDEBIT ");
				queryString.append(" ,CASE WHEN ( ");
				queryString.append(" (ACTYPE IN ('I','L')  AND CC." + reportBudgetMonth + " > 0 ) OR ");
				queryString.append(" (ACTYPE IN ('A','E')  AND CC." + reportBudgetMonth + " < 0 ) ");
				queryString.append(" ) THEN ABS(CC." + reportBudgetMonth + ") ELSE 0 END AS mCREDIT ,0 AS DEBIT,0 AS CREDIT ");
				...
				queryString.append("SELECT C.ACTYPE AS ACTYPE, CC.DEPARTMENTID  , C.ACCODE AS ACODE  , C.ACNAME AS ACNAME ");
				queryString.append(" ,CASE WHEN  ");
				//queryString.append(" (ACTYPE IN ('I','L')  AND CC." + thisMonth + " < 0 ) OR ");
				...
				//queryString.append(" (ACTYPE IN ('A','E')  AND CC." + thisMonth + " < 0 ) ");
				queryString.append("  THEN (CC."+thisMonth+" - " +"CC."+ previousBudgetMonth + ") ELSE 0 END AS mCREDIT ,0 AS DEBIT,0 AS CREDIT ");
				queryString.append(" FROM COA AS C INNER JOIN VW_CCOA AS CC ON C.ID=CC.COAID WHERE CC.BUDGET=?budget ");
				/*queryString.append("SELECT C.ACTYPE AS ACTYPE, CC.DEPARTMENTID  , C.ACCODE AS ACODE  , C.ACNAME AS ACNAME ");
				queryString.append(" ,CASE WHEN ( ");
				queryString.append(" (ACTYPE IN ('I','L')  AND CC." + previousBudgetMonth + " < 0 ) OR ");
				queryString.append(" (ACTYPE IN ('A','E')  AND CC." + previousBudgetMonth + " > 0 ) ");
				queryString.append(" ) THEN ABS(CC." + previousBudgetMonth + ") ELSE 0 END AS mDEBIT ");
				queryString.append(" ,CASE WHEN ( ");
				queryString.append(" (ACTYPE IN ('I','L')  AND CC." + previousBudgetMonth + " > 0 ) OR ");
				queryString.append(" (ACTYPE IN ('A','E')  AND CC." + previousBudgetMonth + " < 0 ) ");
				queryString.append(" ) THEN ABS(CC." + previousBudgetMonth + ") ELSE 0 END AS mCREDIT ,0 AS DEBIT,0 AS CREDIT ");
				queryString.append(" FROM COA AS C INNER JOIN VW_CCOA AS CC ON C.ID=CC.COAID WHERE CC.BUDGET=?budget ");*/
				...
				if (dto.isGroup()) {
					...
					queryString.append(" AND C.ACCODETYPE='DETAIL' ");
					//queryString.append(" AND C.ACCODETYPE='DETAIL' AND C.ACCODE='01-001-004' ");
				}
